...
def main():
    ...
    val_loader = DataLoader(cityscapes(args.datadir, input_transform_cityscapes, target_transform_cityscapes, subset=args.subset), num_workers=args.num_workers, batch_size=args.batch_size, shuffle=False)

    
    temp_model.set_temperature(val_loader)
    ...
    if len(ood_gts_list) > 0 and len(anomaly_score_list) > 0:
        if np.any(ood_mask) and np.any(ind_mask):
            ...
            print(f'Temperature: {temp_model.temperature} - AUPRC: {prc_auc_msp * 100.0}, FPR@TPR95: {fpr_msp * 100.0}')
